...
function SearchUserModal({ show }) {
    ...
    const updateUsers = () => {
        if ((name !== null || name !== '') && (userName === null || userName === '')) {
            fetch(`https://find-luv.onrender.com/api/single/${name}`)
                .then(response => response.json())
                .then(data => addSingleUser(data[0]))
                .then(() => console.log(singleUser))
        }
        if ((name === null || name === '') && (userName !== null || userName === '')) {
            fetch(`https://find-luv.onrender.com/api/single/${userName}`)
                .then(response => response.json())
                // .then(data => console.log(data))
                .then(data => addSingleUser(data[0]))
        }
        if ((name !== null || name !== '') && (userName !== null || userName !== '')) {
            fetch(`https://find-luv.onrender.com/api/single/${name}/${userName}`)
                .then(response => response.json())
                // .then(data => console.log(data))
                .then(data => addSingleUser(data[0]))
        }
